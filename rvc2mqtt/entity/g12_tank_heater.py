...
class DimmerSwitch_DC_DIMMER_STATUS_3(EntityPluginBaseClass):
    FACTORY_MATCH_ATTRIBUTES = {"name": "DC_DIMMER_STATUS_3", "type": "g12_tank_heater"}
    """
    The tank heaters connected to the G12 use the RVC DGN of DC_DIMMER_STATUS_3 and DC_DIMMER_COMMAND_2
    The firefly panel always does a toggle command. In order to support turning off individual heater
    and all in a group On/Off is used instead to avoid heaters states getting out of sync.
    Supports On/Off
    ...
    def __init__(self, data: dict, mqtt_support: MQTT_Support):
        self.id = "tank_heaters" + str(data["instance"])
        super().__init__(data, mqtt_support)
        self.Logger = logging.getLogger(__class__.__name__)

        # Allow MQTT to control tank heaters
        ...
        self.tank_heater_entity_link = []

        """
        if self.tank_heater_entity_link:
    ...
    def add_entity_link(self, obj):
        ...
        self.Logger.info(f"link object: {str(dir(obj))}")
        self.tank_heater_entity_link = obj
