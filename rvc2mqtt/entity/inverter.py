...
class InverterCharger_INVERTER_STATUS(EntityPluginBaseClass):
    ...
    def __init__(self, data: dict, mqtt_support: MQTT_Support):
        ...
        self.rvc_match_inverter_temperature_status = { "name": "INVERTER_TEMPERATURE_STATUS", "instance": self.rvc_instance}
    ...
    def process_rvc_msg(self, new_message: dict) -> bool:
        ...
        if self._is_entry_match(self.rvc_match_inverter_status, new_message):
            ...
            self.publish(self.status_def_topic,
                new_message.get("status_definition", "unknown").title())
            if int(self.status) == 0:
                self.onoff = "off"
            elif int(self.status) > 0:
                self.onoff = "on"
            self.publish(self.onoff_topic, self.onoff)

            self.publish(self.batt_sensor_pres_topic,
                new_message["battery_temperature_sensor_present"])
            self.publish(self.batt_sensor_pres_def_topic,
                new_message.get("battery_temperature_sensor_present_definition", "unknown").title())
            ...
            self.publish(_volt_topic, _volt)

            self.publish(_curr_topic, _curr)

            self.publish(_freq_topic, _freq)

            self.publish(_f_o_g_topic, _f_o_g)

            self.publish(_f_o_n_topic, _f_o_n)

            self.publish(_f_r_p_topic, _f_r_p)

            self.publish(_f_g_c_topic, _f_g_c)
            ...
            self.publish(_gcur_topic, _gcur)

            self.publish(_cap_topic, _cap)
            ...
            self.publish(_wave_topic, _wave)
            self.publish(_wave_def_topic, _wave_def)

            self.publish(_phase_topic, _phase)
            self.publish(_phase_def_topic, _phase_def)

            self.publish(_realp_topic, _realp)

            self.publish(_reactp_topic, _reactp)

            self.publish(_harmd_topic, _harmd)

            self.publish(_compleg_topic, _compleg)
            ...
            self.publish(_f_volt_topic, _f_volt)
            self.publish(_f_volt_def_topic, _f_volt_def)

            self.publish(_f_surge_topic, _f_surge)
            self.publish(_f_surge_def_topic, _f_surge_def)

            self.publish(_f_hfreq_topic, _f_hfreq)
            self.publish(_f_hfreq_def_topic, _f_hfreq_def)

            self.publish(_f_lfreq_topic, _f_lfreq)
            self.publish(_f_lfreq_def_topic, _f_lfreq_def)

            self.publish(_f_bypas_topic, _f_bypas)
            self.publish(_f_bypas_def_topic, _f_bypas_def)

            self.publish(_f_qual_topic, _f_qual)
            self.publish(_f_qual_def_topic, _f_qual_def)
            ...
            self.publish(self.dc_voltage_topic, new_message["dc_voltage"])

            self.publish(self.dc_amperage_topic, new_message["dc_amperage"])
            ...
            self.publish(self.fet_1_temperature_topic, new_message["fet_1_temperature"])

            self.publish(self.transformer_temperature_topic, new_message["transformer_temperature"])

            self.publish(self.fet_2_temperature_topic, new_message["fet_2_temperature"])
