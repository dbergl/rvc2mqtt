...
class VegatouchMira(EntityPluginBaseClass):
    FACTORY_MATCH_ATTRIBUTES = {"name": "VEGATOUCH_MIRA", "type": "vegatouch_mira"}

    """
    Entity for the Vegatouch Mira control panel.

    Listens to INITIAL_PACKET / DATA_PACKET multi-packet transport filtered
    by source_id (Mira broadcasts on 0xFD) and publishes PRODUCT_IDENTIFICATION.
    Also handles DM_1 (off-spec DM_RV alias used by Spyder Controls) for fault reporting.

    Floorplan entry example:
      - name: VEGATOUCH_MIRA
        type: vegatouch_mira
        source_id: 'FD'
        instance_name: "Vegatouch Mira"
        status_topic: "rvc2mqtt/mira"
    """

    def __init__(self, data: dict, mqtt_support: MQTT_Support):
        self.id = "vegatouch-mira-" + str(data['source_id'])
        ...
        self.rvc_match_dm_1           = {"name": "DM_1",           "source_id": self.source_id}
        ...
        if 'status_topic' in data:
            ...
            self.dm_1_fault_code_topic     = str(f"{topic_base}/fault/code")
            self.dm_1_fault_description_topic = str(f"{topic_base}/fault/description")
            self.dm_1_lamp_topic           = str(f"{topic_base}/fault/lamp")
    ...
    def process_rvc_msg(self, new_message: dict) -> bool:
        ...
        if self._is_entry_match(self.rvc_match_dm_1, new_message):
            self.Logger.debug(f"Msg Match DM_1: {str(new_message)}")
