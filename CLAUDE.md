...
- **`virtual_bms.py`** — Standalone safety-critical relay for the APS-500 alternator regulator's proprietary BMS link (0EF70/0EF80, see `rvc-spec.yml`). Relays the real Renogy BMS's native protocol (can0) into that link on can1 while letting specific alarm categories be suppressed at level 1 only (e.g. to let a low-temperature alarm through just far enough to run the battery's own heater relay). Requires the real BMS physically disconnected from can1 (it stays on can0). Run manually: `python3 tools/virtual_bms.py --can0 can0 --can1 can1`; control via MQTT (`rvc/virtualbms/set/override`, status on `rvc/virtualbms/status`). Not wired into the plugin/entity system — deliberately a manual-use tool, not part of the normal running service.
